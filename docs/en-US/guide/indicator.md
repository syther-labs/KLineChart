...
<Tip title="Tip" :tip="['Some indicators can be overlaid on candles using <code>chart.createIndicator({ name: \'MA\', paneId: \'candle_pane\' }, true)</code>, while some cannot. Built-in candle-compatible indicators are: BBI, BOLL, EMA, MA, SAR, SMA.']"/>
